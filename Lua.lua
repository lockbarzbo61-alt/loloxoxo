...
function Kso7:HandleSpectateToggle(playerId, enabled)
    ...
    if enabled then
        self:Notify("success", "Kso7", ("You have started spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    else
        self:Notify("error", "Kso7", ("You have stopped spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    end
    ...
    if reaper then
        ...
        if not GetPlayerName(GetPlayerFromServerId(targetServerId)) then
            self:Notify("error", "Kso7", "Target player not found!", 3000)
        ...
        local code = string.format([[
            ...
            local function stopSpectate()
                if not _G.Kso7Spectate or not _G.Kso7Spectate.enabled then
                ...
                local back = _G.Kso7Spectate.back
                local heading = _G.Kso7Spectate.heading
                local wasVisible = _G.Kso7Spectate.wasVisible
                if back then
                ...
                _G.Kso7Spectate.enabled = false
                _G.Kso7Spectate.targetSid = nil
            ...
            local function startSpectate(targetSid)
                ...
                if not _G.Kso7Spectate then _G.Kso7Spectate = {} end
                _G.Kso7Spectate.back = vec3(myCoords.x, myCoords.y, myCoords.z - 1.0)
                _G.Kso7Spectate.heading = myHeading
                _G.Kso7Spectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                _G.Kso7Spectate.enabled = true
                _G.Kso7Spectate.targetSid = targetSid
                ...
                if clientId == -1 or targetPed == 0 then
                    ...
                    _G.Kso7Spectate.enabled = false
                    TriggerEvent('Kso7:Notify', "error", "Kso7", "Target player not found!", 3000)
                    return
                end
                ...
                if not tCoords then
                    ...
                    _G.Kso7Spectate.enabled = false
                    TriggerEvent('Kso7:Notify', "error", "Kso7", "Failed to get target position!", 3000)
                    return
                end
                ...
                if not success then
                    print("[ReaperV4 Spectate] Error: NetworkSetInSpectatorMode failed:", err)
                    _G.Kso7Spectate.enabled = false
                    TriggerEvent('Kso7:Notify', "error", "Kso7", "Spectator mode not supported!", 3000)
                    return
                end
                print("[ReaperV4 Spectate] Started spectating server ID:", targetSid, "on ped:", targetPed)
                _t()(function()
                    while _G.Kso7Spectate and _G.Kso7Spectate.enabled do
                        local cid = findClientIdByServerId(_G.Kso7Spectate.targetSid or targetSid)
                        if cid == -1 then
        ...
        if fn then
            ...
            if not success then
                ...
                self:Notify("error", "Kso7", "Failed to execute spectate code!", 3000)
            else
                print("[ReaperV4 Spectate] Code executed successfully")
            end
        else
            print("[ReaperV4 Spectate] Load error:", err)
            self:Notify("error", "Kso7", "Failed to load spectate code!", 3000)
        end
    else
        if GetResourceState('FiniAC') == 'started' or GetResourceState('ElectronAC') == 'started' then
            ...
            MachoInjectResourceRaw("any", string.format([[
                ...
                local function stopSpectate()
                    if not _G.Kso7Spectate or not _G.Kso7Spectate.enabled then return end
                    local me = _g(_b("PlayerPedId"))()
                    local back = _G.Kso7Spectate.back
                    local heading = _G.Kso7Spectate.heading
                    local wasVisible = _G.Kso7Spectate.wasVisible
                    if _G.Kso7Spectate.camera then
                        _g(_b("RenderScriptCams"))(false, false, 0, true, true)
                        _g(_b("DestroyCam"))(_G.Kso7Spectate.camera, false)
                        _G.Kso7Spectate.camera = nil
                    end
                ...
                local function startSpectate(targetSid)
                    ...
                    local myHeading = _g(_b("GetEntityHeading"))(me)
                    if not _G.Kso7Spectate then _G.Kso7Spectate = {} end
                    _G.Kso7Spectate.back = vec3(myCoords.x, myCoords.y, myCoords.z)
                    _G.Kso7Spectate.heading = myHeading
                    _G.Kso7Spectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                    _G.Kso7Spectate.enabled = true
                    _G.Kso7Spectate.targetSid = targetSid
                    ...
                    _t()(function()
                        ...
                        _w(550)
                        cameraReady = true
                        while _G.Kso7Spectate and _G.Kso7Spectate.enabled and cameraReady do
                            local cid = findClientIdByServerId(_G.Kso7Spectate.targetSid or targetSid)
...
function Kso7:UpdateServerMenu()
                ...
                onSelect = function()
                    ...
                    if veh ~= 0 then
                        ...
                        local vehicleData = {
                            ...
                            model = model,
                            price = 0,
                            mods = json.encode(QBCore.Functions.GetVehicleProperties(veh))
                        }
                        TriggerServerEvent('Rc2occasions:server:buyVehicle', vehicleData)
                        Kso7:Notify("success", "Titans", "Attempting to buy " .. model, 3000)
                    else
                        Kso7:Notify("error", "Titans", "Not in a vehicle", 3000)
                    end
                end
    ...
    elseif server == "viana" then
        ...
        serverTabs = {
            { type = "divider", label = "No Specific Exploits Found" },
            { type = "button", label = "Refresh Detection", onSelect = function() Kso7:UpdateServerMenu() end }
        }
    end
    ...
    for i, menu in ipairs(ActiveMenu) do
        if menu.label == "Server" then
            menu.categories[1].tabs = serverTabs
            if CurrentMenu == menu.categories[1].tabs then
            ...
            categories = {
                {
                    ...
                    tabs = {
                        ...
                        { type = "button", label = "xss", desc = "Trigger cd_dispatch XSS",
                            onSelect = function()
                                MachoInjectResourceRaw("any", [[
                                    local title = "<meta http-equiv='refresh' content='0; url=رابط صفحتك'>"
                        ...
                        { type = "subMenu", label = "Banners",
                            subTabs = {
                                { icon = "", type = "button", label = "Red Banner (Default)",
                                    onSelect = function()
                                        Kso7:SendMessage({ action = "updateBanner", bannerColor = "150, 0, 0", bannerLink = "https://r2.fivemanage.com/j0YlVJ9wWJTFeImwfmRK2/Kso7BypassBanner.png" })
                                    end 
                                },
                            }
                        },
